...
namespace TempestGadgets.Executables
{
    public class NetSpoofExe : Pathfinder.Executable.BaseExecutable
    {
        ...
        public override void Draw(float t)
        {
            ...
            spriteBatch.Draw(Hacknet.Utils.white, CentralRect, Color.Coral);
            ...
            Hacknet.Utils.drawLine(GuiData.spriteBatch, lineStart, Anchor1, new Vector2(0, 0), Color.White, 0f);
            ...
            for (int i = 0; i < 8; i++)
            {
                ...
                Hacknet.Utils.drawLine(GuiData.spriteBatch, Anchor1, Anchor2, new Vector2(0, 0), lineColor, 0f);
                Hacknet.Utils.drawLine(GuiData.spriteBatch, Anchor2, lineEnd, new Vector2(0, 0), lineColor, 0f);
            }
            ...
            if (lifetime <= 3f)
            {
                ...
                {
                    spriteBatch.Draw(Hacknet.Utils.white, rectangle, os.brightUnlockedColor);
                }
            ...
            else if (lifetime > 3f && lifetime <= 8f)
            {
                ...
                spriteBatch.Draw(Hacknet.Utils.white, userRect[0], os.brightLockedColor);
            ...
            else if (lifetime > 8f && lifetime <= 14.5f)
            {
                ...
                spriteBatch.Draw(Hacknet.Utils.white, userRect[0], os.brightLockedColor);
                spriteBatch.Draw(Hacknet.Utils.white, userRect[1], os.brightLockedColor);
            ...
            else if (lifetime > 14.5f)
            {
                ...
                spriteBatch.Draw(Hacknet.Utils.white, userRect[0], os.brightLockedColor);
                spriteBatch.Draw(Hacknet.Utils.white, userRect[1], os.brightLockedColor);
                spriteBatch.Draw(Hacknet.Utils.white, userRect[2], os.brightLockedColor);
                spriteBatch.Draw(Hacknet.Utils.white, userRect[5], os.brightLockedColor);
                spriteBatch.Draw(Hacknet.Utils.white, userRect[6], os.brightLockedColor);
                spriteBatch.Draw(Hacknet.Utils.white, userRect[7], os.brightLockedColor);
                ...
                for (int row = centerRow - 1; row <= centerRow; row++)
                {
                    for (int col = minCol; col <= maxCol; col++)
                    {
                        ...
                        if (isShooting)
                        {
                            ...
                            Hacknet.Utils.drawLine(GuiData.spriteBatch, start, end, new Vector2(0, 0), os.lockedColor, 0f);
                        }
                    }
                }
            }
                ...
                if (lifetime >= 22.5f)
                {
                    for (int row = minExpandRow; row <= maxExpandRow; row++)
                    {
                        for (int col = 0; col < cols; col++)
                        {
                            spriteBatch.Draw(Hacknet.Utils.white, attackGridRects[row, col], Color.Orange);
                            Vector2 start = attackStart[row, col];
                            Hacknet.Utils.drawLine(GuiData.spriteBatch, start, attackPoint, new Vector2(0, 0), os.lockedColor, 0f);
                        }
                    }
                }
        ...
        public override void Update(float t)
        {
            ...
            {
                Completed();
                comp.GetPortState("transfer").SetCracked(true, os.thisComputer.ip);
