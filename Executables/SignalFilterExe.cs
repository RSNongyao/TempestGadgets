...
public class SignalFilterExe : Pathfinder.Executable.BaseExecutable
{
    ...
    // 横幅显示文本
    private const string WARNING_TEXT = "WARNING: ACTIVE PORT CRACKING";
    ...
    public override void Update(float t)
    {
        ...
        if (!isComplete)
        {
            ...
            if (lifetime >= 15f)
            {
                ...
                os.write("Port SignalScramble Opened!");
                os.write("Keep this running until disconnect!!!");

                RandomizePorts();

                RandomizeProxyFirewall();
            }
        }
        else
        {
    ...
    private void RandomizePorts()
    {
        ...
        Console.WriteLine($"Randomizing {OperationsCount} ports on {c.ip}...");
        ...
        for (int i = 0; i < OperationsCount; i++)
        {
            ...
            Console.WriteLine($"Randomizing port {targetPort} on {c.ip}...");
    ...
    private void RandomizeProxyFirewall()
    {
        Computer c = ComputerLookup.FindByIp(targetIP);

        if (c == null) return;

        if (c.proxyActive || c.firewall != null)
        {
            Random random = new Random();
            int randomNumber = random.Next(1, 5);

            if (randomNumber == 2) c.firewall = null;
            else if (randomNumber == 3) c.proxyActive = false;
